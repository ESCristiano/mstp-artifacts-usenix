...
static int dump_mpi_hex_full(const char *tag,
                                const mbedtls_mpi *X,
                                size_t hex_digits)
{
    /* how many bytes do we need for that many hex digits?             */
    size_t want_len = (hex_digits + 1) / 2;          /* +1 ⇒ ceiling div2 */
    if (want_len > MBEDTLS_MPI_MAX_SIZE)
        return MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL;

    unsigned char buf[MBEDTLS_MPI_MAX_SIZE];
    memset(buf, 0, want_len);                        /* leading-zero pad  */

    /* write the MPI at the *right-hand* end of the buffer             */
    size_t len = mbedtls_mpi_size(X);
    if (len > want_len)                               /* value too large  */
        return MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL;

    int ret = mbedtls_mpi_write_binary(X,
                                       buf + (want_len - len),   /* dest   */
                                       len);                     /* bytes  */
    if (ret != 0) return ret;

    /* ---------- print as C byte array ----------------------------- */
    printf("uint8_t %s[%u] = { ", tag, want_len);
    printf("\n\r"); 
    for (size_t i = 0; i < want_len; i++) {
        printf("0x%x%s",
               buf[i],
               (i + 1 == want_len) ? " " : ",");
        if ((i + 1) % 16 == 0 && i + 1 < want_len)
            printf("\n\r");               /* wrap every 16 bytes */
    }
    printf("};\n\r");
    printf("\n\r");
    return 0;
...
/* Captured values produced during key-generation (1024-bits RSA) */
const uint32_t rsa_key_bits = 1024;

uint8_t Saved_P[64] = {
    0xe2,0xd3,0x12,0x8e,0x5a,0xb,0x29,0xee,0x37,0x7e,0x3e,0xdf,0xe8,0x44,0x85,0xf3,
    0x1b,0x93,0x80,0xf0,0x2b,0xf4,0xab,0x32,0x51,0xa7,0xd3,0xb3,0x7,0x71,0x2d,0x4,
    0x0,0x40,0x5b,0x75,0xf0,0x4,0x3d,0x46,0x1e,0x6a,0xd3,0x20,0x85,0xae,0x86,0x92,
    0x8e,0x89,0x16,0xa1,0xb8,0xe4,0xb7,0x4d,0x32,0xff,0x7f,0x51,0xff,0x6,0xb7,0x83};
    
uint8_t Saved_Q[64] = {
    0xb6,0xbd,0xd5,0x31,0x1a,0xbe,0x8d,0x92,0x75,0x65,0x18,0x8f,0x2b,0xdf,0x3c,0xe7,
    0xc,0x50,0x71,0x8b,0xff,0xc3,0xa,0xb4,0x99,0x7b,0x2d,0xca,0x39,0x38,0xa7,0x8c,
    0xbc,0xa9,0x2b,0x5e,0x2a,0xfa,0x64,0x14,0xd2,0x17,0xd7,0xdc,0x95,0x86,0x47,0x2,
    0x7c,0xae,0x12,0x15,0xa5,0x3,0x90,0x9d,0xb8,0x3b,0xf,0xfa,0x88,0x8c,0x3b,0x73};
    
uint8_t Saved_D[128] = {
    0x16,0xe4,0x7a,0x21,0x72,0xd9,0x58,0x1,0xb8,0xf9,0xfe,0xdd,0x9c,0x7c,0xb0,0x4c,
    0x0,0x1,0x2a,0x3c,0x12,0x4f,0xd,0xd3,0x7e,0x12,0x5a,0x2b,0x21,0x5b,0x9a,0xb5,
    0xf,0xa4,0x3d,0x7e,0x59,0xc3,0x92,0x49,0x3c,0xc8,0x41,0x38,0xbb,0x9a,0xe7,0x60,
    0xb5,0xd3,0xce,0xc0,0x51,0xc4,0x67,0x15,0x33,0xd4,0x91,0x6e,0xd6,0x9,0x8a,0x2f,
    0xaf,0xf5,0x8e,0x31,0xd5,0xe6,0xcf,0xac,0x6,0x2,0x78,0x8a,0xa4,0x5f,0xe1,0x8d,
    0x51,0x6f,0x1d,0x47,0x69,0x1c,0x8b,0x9b,0x6a,0x2c,0xc8,0x82,0x76,0x71,0xe5,0x32,
    0x88,0x74,0x61,0xc6,0x26,0x34,0xe2,0x75,0xbc,0x2e,0x8e,0xd4,0x7d,0xf2,0x60,0xe0,
    0x63,0x82,0xd6,0xc7,0xd7,0x3f,0xb0,0xfb,0x81,0x9a,0x16,0x99,0x34,0xcd,0x6,0x89};
    
uint8_t Saved_DP[64] = {
    0x6,0x81,0xb7,0x8e,0x8d,0xd6,0xb4,0x25,0x39,0x42,0x4d,0xdb,0x6e,0x76,0x48,0xc1,
    0x68,0x91,0x1a,0xa9,0xc9,0x19,0x3,0xb0,0x35,0xd7,0x72,0xa3,0x38,0x1b,0x6f,0x7,
    0x26,0x5a,0xb2,0x44,0xf7,0xd5,0xa7,0x4c,0x53,0xa3,0xc,0xdb,0x69,0xfa,0x4f,0xb1,
    0xf4,0x94,0xca,0x29,0x69,0x7c,0x66,0x25,0xb8,0xc8,0xcb,0x86,0x36,0xa2,0xa2,0xb1};
    
uint8_t Saved_DQ[64] = {
    0x2c,0x81,0x55,0x1f,0xad,0x6c,0xc5,0x1b,0xeb,0x3d,0x60,0xda,0xd5,0x8b,0x7b,0xd0,
    0xa5,0x7e,0x1d,0xf9,0x43,0xa3,0xe3,0xc2,0x92,0x32,0x9e,0xb7,0x46,0x72,0x56,0xd3,
    0xfe,0xca,0x90,0x86,0x51,0x9f,0x43,0xaa,0x92,0x74,0x3e,0x1d,0x39,0xc9,0x47,0x18,
    0x2,0xf7,0x7d,0xee,0x9,0x8f,0x31,0xb4,0x47,0xbf,0x12,0xa8,0x28,0x83,0x46,0xa3};
    
uint8_t Saved_QP[64] = {
    0x1e,0x5e,0xf2,0xda,0xdb,0x57,0x40,0x5d,0xfd,0xb0,0x97,0xee,0x7,0xd9,0x2a,0x2c,
    0x8f,0x7d,0x20,0x4a,0x44,0x9,0xfe,0x7a,0x11,0x1f,0xc2,0xaf,0xdd,0x6e,0x7d,0xb2,
    0x13,0x54,0x87,0x5a,0x9,0x59,0x64,0x75,0x17,0x8c,0x58,0xd2,0x58,0xf2,0xcc,0x62,
    0x5d,0xfd,0xae,0x30,0x98,0xf0,0x32,0x92,0x35,0x8d,0x6e,0xee,0x9,0x6f,0x2b,0xa6};
    

size_t   prime_hex = rsa_key_bits / 8;   /* 128 */
size_t   d_hex     = rsa_key_bits / 4;   /* 256 */

int rsa_deduce_crt_poc(void)
{
    int ret;
    ...
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary(&P, Saved_P, sizeof Saved_P) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary(&Q, Saved_Q, sizeof Saved_Q) );
    MBEDTLS_MPI_CHK( mbedtls_mpi_read_binary(&D, Saved_D, sizeof Saved_D) );
    ...
    // printf("T4\r\n");
    if (ret != 0) {
        printf("deduce_crt failed: %d\r\n", ret);
        goto cleanup;
    }

    // printf("T5\r\n");
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("Saved_DP",&DP,prime_hex) );
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("DP",&DP,prime_hex) );
    
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("Saved_DQ",&DQ,prime_hex) );
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("DQ",&DQ,prime_hex) );
    
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("Saved_QP",&QP,prime_hex) );
    // MBEDTLS_MPI_CHK( dump_mpi_hex_full("QP",&QP,prime_hex) );
...
{
    #ifdef RSA_GENKEY_POC_TEST
        rsa_deduce_crt_poc(); // for testing the CRT deduce function
    #else
    //     int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    //     mbedtls_mpi H, G, L;
    //     int prime_quality = 0;
    
    //     printf("\n\rmbedtls_rsa_gen_key: RSA key size %u\n\r", nbits);
    ...
    //     * QP = Q^-1 mod P
    //     */
    //     dump_mpi_hex_full("Saved_P", &ctx->P, prime_hex);
    //     dump_mpi_hex_full("Saved_Q", &ctx->Q, prime_hex);
    //     dump_mpi_hex_full("Saved_D", &ctx->D, d_hex);
        
    //     MBEDTLS_MPI_CHK(mbedtls_rsa_deduce_crt(&ctx->P, &ctx->Q, &ctx->D,
    //                                         &ctx->DP, &ctx->DQ, &ctx->QP));
    //     dump_mpi_hex_full("Saved_DP", &ctx->DP, prime_hex);
    //     dump_mpi_hex_full("Saved_DQ", &ctx->DQ, prime_hex);
    //     dump_mpi_hex_full("Saved_QP", &ctx->QP, prime_hex);
